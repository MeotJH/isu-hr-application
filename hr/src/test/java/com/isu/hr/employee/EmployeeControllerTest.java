...
public class EmployeeControllerTest extends RestDocsTestBase {
    ...
    @Test
    @DisplayName("직원들 가져오기 API - 정상 응답")
    void getAllEmployeesTest() throws Exception {
        ...
        when(service.getAllEmployees(sabuns)).thenReturn(responseDtos);

        ObjectMapper objectMapper = new ObjectMapper();
        String requestJson = objectMapper.writeValueAsString(sabuns);
        ...
        mockMvc.perform(post("/employee/list")
                ...
                .andExpect(jsonPath("$[1].email").value("park@test.com"))
                .andDo(document("employee/list",
                        requestFields(
                                fieldWithPath("[]").description("조회할 직원들의 사번 목록")
                        ),
                        responseFields(
                                fieldWithPath("[].sabun").description("조회된 직원의 사번"),
                                fieldWithPath("[].name").description("조회된 직원의 이름"),
                                fieldWithPath("[].birYmd").description("조회된 직원의 생년월일"),
                                fieldWithPath("[].empYmd").description("조회된 직원의 입사일"),
                                fieldWithPath("[].email").description("조회된 직원의 이메일 주소"),
                                fieldWithPath("[].address").description("조회된 직원의 거주지")
                        )));
    }
    ...
    @Test
    @DisplayName("직원 수정하기 API - 정상 응답")
    void modifyEmployeeTest() throws Exception {
        //given
        List<EmployeeRequestDto> requestDtos = List.of(
                EmployeeRequestDto.builder()
                        .sabun("10001")
        ...
        when(service.modifyEmployee(any())).thenReturn(responseDtos);
        ...
        mockMvc.perform(patch("/employee")
                ...
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$[0].sabun").value("10001"))
                .andExpect(jsonPath("$[0].name").value("김수정"))
                .andExpect(jsonPath("$[0].email").value("kim.modified@test.com"))
                .andExpect(jsonPath("$[1].sabun").value("10002"))
                .andExpect(jsonPath("$[1].name").value("박수정"))
                .andExpect(jsonPath("$[1].email").value("park.modified@test.com"))
                .andDo(document("employee/modify",
                        requestFields(
                                fieldWithPath("[].sabun").description("수정할 직원의 사번"),
                                fieldWithPath("[].name").description("수정할 직원의 이름"),
                                fieldWithPath("[].birYmd").description("수정할 직원의 생년월일"),
                                fieldWithPath("[].empYmd").description("수정할 직원의 입사일"),
                                fieldWithPath("[].email").description("수정할 직원의 이메일 주소"),
                                fieldWithPath("[].address").description("수정할 직원의 거주지")
                        ),
                        responseFields(
                                fieldWithPath("[].sabun").description("수정된 직원의 사번"),
                                fieldWithPath("[].name").description("수정된 직원의 이름"),
                                fieldWithPath("[].birYmd").description("수정된 직원의 생년월일"),
                                fieldWithPath("[].empYmd").description("수정된 직원의 입사일"),
                                fieldWithPath("[].email").description("수정된 직원의 이메일 주소"),
                                fieldWithPath("[].address").description("수정된 직원의 거주지")
                        )));
    }

    @Test
    @DisplayName("직원 삭제하기 API - 정상 응답")
    void deleteEmployeeTest() throws Exception {
        ...
        when(service.deleteEmployee(any())).thenReturn(responseDtos);
        ...
        mockMvc.perform(delete("/employee")
                ...
                .andDo(document("employee/delete",
                        requestFields(
                                fieldWithPath("[].sabun").description("삭제할 직원의 사번"),
                                fieldWithPath("[].name").description("삭제할 직원의 이름"),
                                fieldWithPath("[].birYmd").description("삭제할 직원의 생년월일"),
                                fieldWithPath("[].empYmd").description("삭제할 직원의 입사일"),
                                fieldWithPath("[].email").description("삭제할 직원의 이메일 주소"),
                                fieldWithPath("[].address").description("삭제할 직원의 거주지")
                        ),
                        responseFields(
                                fieldWithPath("[].sabun").description("삭제된 직원의 사번"),
                                fieldWithPath("[].name").description("삭제된 직원의 이름"),
                                fieldWithPath("[].birYmd").description("삭제된 직원의 생년월일"),
                                fieldWithPath("[].empYmd").description("삭제된 직원의 입사일"),
                                fieldWithPath("[].email").description("삭제된 직원의 이메일 주소"),
                                fieldWithPath("[].address").description("삭제된 직원의 거주지")
                        )));
    }
}
